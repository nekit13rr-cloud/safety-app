...
export default function ManagerDashboard() {
  ...
  const applyFilters = () => {
    ...
    if (siteFilter) filtered = filtered.filter(o => o.site_id.toString() === siteFilter);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      setToast({ message: 'Excel файл скачан', type: 'success' });
    } catch (err) {
      console.error('Ошибка:', err);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `observations_${new Date().toISOString().split('T')[0]}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      
      setToast({ message: 'PDF файл скачан', type: 'success' });
    } catch (err) {
      console.error('Ошибка:', err);
      setToast({ message: 'Ошибка экспорта', type: 'error' });
    } finally {
